...
export class ProfileComponent implements OnInit {
  ...
  professionInformation: IProfessionInfo[] = [
    { label: 'Naziv', value: '', previousValue: '' },
    { label: 'Biografija', value: '', previousValue: '' },
  ];
  ...
  ngOnInit(): void {
    ...
    this.professionInformation[0].value = responseData.profession;
    this.professionInformation[0].previousValue = responseData.profession;
    this.professionInformation[1].value = responseData.bio;
    this.professionInformation[1].previousValue = responseData.bio;
  ...
  saveProfessionInfo() {
    if (
      this.professionInformation[0].value ===
        this.professionInformation[0].previousValue &&
      this.professionInformation[1].value ===
        this.professionInformation[1].previousValue
    )
      return;

    const professionInfo: IProfession = {
      bio: this.professionInformation[1].value,
      profession: this.professionInformation[0].value,
    };
    this._profileService.changeProfession(professionInfo).subscribe({
      next: (response) => {
        this.professionInformation[0].previousValue =
          this.professionInformation[0].value;
        this.professionInformation[1].previousValue =
  ...
  onFileChange(event: any) {
    ...
    if (files.length > 0) {
      ...
      this._profileService.changeAvatar(fileToUpload).subscribe({
        next: (response) => {
          this.file = URL.createObjectURL(fileToUpload);
        },
        error: (error) => {},
      });
